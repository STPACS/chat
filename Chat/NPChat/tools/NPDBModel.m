...
//+ (BOOL)createTable
//{
//    FMDatabase *db = [FMDatabase databaseWithPath:[NPDBHelper dbPath]];
//    if (![db open]) {
//        NSLog(@"数据库打开失败!");
//        return NO;
//    }
...
+ (BOOL)saveObjects:(NSArray *)array
{
    ...
    for (NPDBModel *model in array) {
        if (![model isKindOfClass:[NPDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            NPDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
